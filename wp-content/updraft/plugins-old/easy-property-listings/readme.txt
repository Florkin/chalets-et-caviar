...
* Compare listings and sales, locationss and leased results.
...
* Multiple listing types are supported because not everyone is a residential properties specialist. You can list locationss, commercial, land, rural and businesses just as easily.
...
* Separate post types for each listing type; property, locations, land, rural, business, commercial, commercial land.
...
1. Create a blank page for each listing type you enable called; property, locations, land, business, rural, commercial, commercial-land. You can rename them after.
...
* New: Disable REST support and Gutenberg by defining new constants for post types. EPL_BUSINESS_DISABLE_REST, EPL_COMMERCIAL_DISABLE_REST, EPL_COMMERCIAL_LAND_DISABLE_REST, EPL_LAND_DISABLE_REST, EPL_PROPERTY_DISABLE_REST, EPL_locations_DISABLE_REST, EPL_RURAL_DISABLE_REST.
...
* New: Filters added for listing admin columns allowing other plugins to hook in correctly to listing dashboard columns and display additional info like Yoast SEO, Post Counter and many other WordPress plugins: epl_post_type_business_admin_columns, epl_post_type_commercial_admin_columns, epl_post_type_commercial_land_admin_columns, epl_post_type_land_admin_columns, epl_post_type_property_admin_columns, epl_post_type_locations_admin_columns, epl_post_type_rural_admin_columns.
...
* Tweak: Search better supports multiple post types in an array: epl_get_unique_post_meta_values('property_bedroom','current', array('property','locations') ).
...
* Fix: Sorting by locations price corrected in admin for locations listings.
...
* Fix: Sorting locationss after performing search would sometimes return no results.
...
* Fix: locations sorting error in listing shortcodes.
...
* New: Pet Friendly option added to locationss.
...
* Tweak: Adjusted price and locations search ranges.
...
* Tweak: Leased locations listings now display the weekly rent amount in admin.
...
* Tweak: Listing price, sale, and locations price now supports decimal values when saving.
...
* Fix: locations listing when using price text the locations period no longer displays in admin.
...
* New: locations display or hide locations price.
...
* Fix: [listing post_type="locations"] shortcode price sorting for locations.
...
* Fix: Widget leased selection displays locationss correctly.
...
* Fix: Corrected sorting by price when using shortcodes. Note: locations sorting works on post_type="locations" in all shortcodes.
* Tweak: Added locations rate view for text entry of locations rates for REAXML compatibility.
...
* Tweak: locations Listing: Added epl_property_bond_position filter to adjust the position of the Bond/Deposit to appear either before or after the value.
* Tweak: locations Listing: Removed CSS padding before bond value.
* Fix: locations Listing: Adjusting the Bond/Deposit label will now show your custom label in the locations Price box.
...
* Tweak: New filter epl_price_number_format added for decimal locations rates.
...
* New: Added date picker for available date on locations listing.
...
* New: Commercial locations lease duration now selectable.
* New: Rooms field added to set the number of rooms that the listing has.
* New: Date listed field added to all listing types.
* New: Year built field added to property, locations, rural listing types.
...
* Tweak: Commercial locations rate now accepts decimal numbers.
...
* New: Search Widget - Labels are configurable from the Display settings allowing you to set for example: "Property" to "Buy" and "locations" to "Rent" and use a single widget to search multiple types.
...
* New: Display: Added customisable label for locations Bond/Deposit.
...
* New: Graph in admin allows you to set the max bar graph value. Default are (2,000,000 sale) and (2,000 locations).
...
* Tweak: Added better Bond/Deposit for locationss labels.
...
* Fix: No longer show Bond when viewing locations list in admin.
* Fix: Open for inspection sticker now appears on locations properties.
...
* New: Added rent period, Day, Daily, Month, Monthly to locations listing types.
