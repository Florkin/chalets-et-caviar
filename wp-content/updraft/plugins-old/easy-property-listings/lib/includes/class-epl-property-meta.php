...
class EPL_Property_Meta {
	 ...
	 *
	 * @since 2.0
	 * @return string Currency Formatted locations Price
	 */
	public function get_property_rent () {
		if($this->post_type != 'locations')
			return;
		...
		if ( $property_rent_search == '') {
			...
			$locations_price = $property_rent_view;
		} else {
			$locations_price = epl_currency_formatted_amount( $property_rent_search );
		}
		return apply_filters('epl_get_property_rent',$locations_price);
	}
	...
	public function get_price_plain_value() {
		...
		} elseif('locations' == $this->post_type) {
	 ...
	 * @return string based on $returntype Options i = span, d = string, l = list item, t = text
	 */
	public function get_property_holiday_locations( $returntype = 'i' ) {

		$label 		= apply_filters('epl_get_property_holiday_locations_label',__('Holiday locations', 'easy-property-listings' ) );
		$value 		= $this->get_property_meta('property_holiday_locations');
		$return 	= '';
		$returntype	= apply_filters( 'epl_get_property_holiday_locations_return_type' , $returntype);
		...
		if( isset($value) && ($value == 1 || $value == 'yes') ) {
			...
			switch( $returntype ) {
				...
				case 'i' :
					$return 	= '<span title="'.$label.'" class="icon holiday_locations"></span>';
					break;
				...
				case 't':
					$return = '<div class="epl-text-icon-container epl-text-icon-container-holiday_locations">
							<span class="epl-text-icon-label holiday_locations">' . $label . '</span>
						</div>';
					break;
				...
				case 'l' :
					$return = '<li class="holiday_locations">'.$label.'</li>';
					break;
			}
			return apply_filters('epl_get_property_holiday_locations',$return,$returntype,$value,$label);
		}
	}
