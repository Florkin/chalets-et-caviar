...
function epl_property_available_dates() {
	global $property;
	if( 'locations' == $property->post_type && $property->get_property_meta('property_date_available') != '' && $property->get_property_meta('property_status') != 'leased' ) {
		// locations Specifics
...
function epl_property_secondary_heading() {
	...
	if ( $property->post_type == 'property' || $property->post_type == 'locations' ) {
...
function epl_property_tab_section() {
	...
	foreach( $general_features_array as $general_feature) {
		...
		switch($general_feature) {
			...
			case 'category':
				...
				if ( 'property' == $post_type || 'locations' == $post_type ) {
	...
	if ( 'property' == $property->post_type || 'locations' == $property->post_type || 'rural' == $property->post_type){
...
function epl_archive_sorting($query) {
	...
	$post_types_locations 	= array('locations');

	if(!$query->is_main_query()){
		return;
	}

	if(is_post_type_archive( $post_types_sold ) || is_post_type_archive( $post_types_locations ) || is_tax ( 'location' ) || is_tax ( 'tax_feature' ) || is_tax ( 'tax_business_listing' ) || epl_is_search() ){
...
function epl_buttons_wrapper_before() {
	echo '<div class="epl-button-wrapper epl-clearfix imgBtnContainer">';
}
