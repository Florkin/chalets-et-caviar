...
function epl_property_author_archives($query) {
	// Add 'locationss' post type to author archives
	if ( $query->is_author )
		$query->set( 'post_type', array('directory', 'locations', 'testimonial' , 'property' , 'post') );
...
function epl_custom_post_author_archive($query) {
	if ($query->is_author)
		$query->set( 'post_type', array('property', 'locations', 'testimonial', 'post', 'commercial' , 'land') );
